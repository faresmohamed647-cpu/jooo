...
for eng, ar in mapping.items():
    ...
    # Escape words and join with \s+ to match multiline HTML text
    words = [re.escape(w) for w in eng.split()]
    pattern_str = r'\s+'.join(words)
    pattern = re.compile(pattern_str)
    
    # Only wrap if it doesn't already have data-i18n
    # We can just substitute the matched string with <span data-i18n="...">matched_string</span>
    # But wait, if the original matched string had tags inside, it will be wrapped by a span, which is fine since span is inline.
    # To prevent double wrapping, we check if key is in html:
